...
class Parameters(object):
    def __init__(self, lmbda=0.1, embedding_size=100, batch_size=100, max_iter=1000, learning_rate=0.1,
                 neg_ratio=1.0, valid_scores_every=50, learning_rate_policy='adam'):
        ...
        self.learning_rate_policy = learning_rate_policy  # Nadam Learning rate policy, see documentation
...
class Batch_Loader_New(object):
    def __init__(self, train_triples, words_indexes, indexes_words, headTailSelector, \
                 entity2id, id2entity, relation2id, id2relation, batch_size, neg_ratio=1.0):
        self.train_triples = train_triples
        self.indexes = np.array(list(self.train_triples.keys())).astype(np.int32)
        self.values = np.array(list(self.train_triples.values())).astype(np.float32)
        self.words_indexes = words_indexes
        ...
        self.new_triples_indexes, self.new_triples_values = self.generateInvalidTriples()

    def __call__(self):

        idxs = np.random.randint(0, len(self.new_triples_indexes), self.batch_size)
        return self.new_triples_indexes[idxs], self.new_triples_values[idxs]

    def generateInvalidTriples(self):

        new_triples_indexes = np.empty((len(self.train_triples) * (self.neg_ratio + 1), 3)).astype(np.int32)
        new_triples_values = np.empty((len(self.train_triples) * (self.neg_ratio + 1), 1)).astype(np.float32)

        new_triples_indexes[:len(self.train_triples), :] = self.indexes
        new_triples_values[:len(self.train_triples)] = self.values

        last_idx = len(self.train_triples)
        ...
        if self.neg_ratio > 0:
            ...
            rdm_words = np.random.randint(0, self.n_words, last_idx * self.neg_ratio)
            # Pre copying everyting
            new_triples_indexes[last_idx:(last_idx * (self.neg_ratio + 1)), :] = np.tile(
                new_triples_indexes[:last_idx, :], (self.neg_ratio, 1))
            new_triples_values[last_idx:(last_idx * (self.neg_ratio + 1))] = np.tile(
                new_triples_values[:last_idx], (self.neg_ratio, 1))

            for i in range(last_idx):
                for j in range(self.neg_ratio):
                    cur_idx = i * self.neg_ratio + j
                    tmpRel = self.indexes_words[new_triples_indexes[last_idx + cur_idx, 1]]
                    tmpIndexRel = self.relation2id[tmpRel]
                    pr = self.headTailSelector[tmpIndexRel]
                    ...
                    if (np.random.randint(np.iinfo(np.int32).max) % 1000) > pr:
                        while (rdm_words[cur_idx] in self.indexes_rels or (
                                rdm_words[cur_idx], new_triples_indexes[last_idx + cur_idx, 1],
                                new_triples_indexes[last_idx + cur_idx, 2]) in self.train_triples):
                            rdm_words[cur_idx] = np.random.randint(0, self.n_words)
                        new_triples_indexes[last_idx + cur_idx, 0] = rdm_words[cur_idx]
                    else:
                        while (rdm_words[cur_idx] in self.indexes_rels or (
                                new_triples_indexes[last_idx + cur_idx, 0],
                                new_triples_indexes[last_idx + cur_idx, 1],
                                rdm_words[cur_idx]) in self.train_triples):
                            rdm_words[cur_idx] = np.random.randint(0, self.n_words)
                        new_triples_indexes[last_idx + cur_idx, 2] = rdm_words[cur_idx]

                    new_triples_values[last_idx + cur_idx] = [-1]

            last_idx += cur_idx + 1

        return new_triples_indexes[:last_idx, :], new_triples_values[:last_idx]
